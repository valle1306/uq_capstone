"""
SGD Baseline Training for Chest X-Ray Classification - 300 Epochs
Matches the original SWAG paper training duration (Maddox et al. 2019)

Key details:
- SGD optimizer with momentum=0.9 (same as SWAG)
- 300 epochs (matching original CIFAR-10 experiments)
- Learning rate schedule: cosine annealing 0.01 → 0.005
- Same weight decay, batch size
- Collection reference: epoch 162 (last 46% of 300 epochs)

This provides the same training duration as the original SWAG paper.
"""
...
def main():
    ...
    # Training (match original SWAG paper - 300 epochs)
    parser.add_argument('--epochs', type=int, default=300,
                        help='number of epochs (default: 300, matching original paper)')
    ...
    parser.add_argument('--swa_start', type=int, default=162,
                        help='epoch for LR schedule reference (default: 162, last 46% of 300)')
    parser.add_argument('--swa_lr', type=float, default=0.005,
                        help='final learning rate (default: 0.005)')
    
    # Misc
    parser.add_argument('--output_dir', type=str, default='results/baseline_sgd_300',
                        help='output directory (default: results/baseline_sgd_300)')
