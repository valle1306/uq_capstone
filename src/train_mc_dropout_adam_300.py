"""
MC Dropout Training with Adam Optimizer for Chest X-Ray Classification - 300 Epochs
...
def main():
    parser = argparse.ArgumentParser(description='MC Dropout Training with Adam - 300 Epochs')
    ...
    # MC Dropout evaluation arguments
    parser.add_argument('--num_samples', type=int, default=30,
                        help='Number of MC dropout samples for inference')
    
    # Output arguments
    parser.add_argument('--save_dir', type=str, default='results/mc_dropout_adam_300')
    ...
    print("="*70)
    print("MC Dropout Training with Adam - 300 Epochs")
    print("="*70)
    ...
    # Create model
    print(f"\nCreating {args.model} with MC Dropout...")
    ...
    model = ResNetWithDropout(base_model, num_classes=2, dropout_rate=args.dropout_rate)
    ...
    print("\n" + "="*50)
    print("Training complete! Now evaluating with MC Dropout...")
    print("="*50)
    ...
    print(f"\nEvaluating with {args.num_samples} MC samples...")
    model.eval()
    ...
    for _ in tqdm(range(args.num_samples), desc='MC Sampling'):
        sample_preds = []
    ...
    mc_acc = 100.0 * (pred_classes == all_labels).mean()
    ...
    print(f"MC Dropout accuracy: {mc_acc:.2f}%")
