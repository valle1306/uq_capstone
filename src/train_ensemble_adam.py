"""
Deep Ensemble Training with Adam Optimizer for Chest X-Ray Classification
...
def train_single_member(member_id, args, device, train_loader, test_loader):
    ...
    print(f"\n{'='*70}")
    print(f"Training Ensemble Member {member_id+1}/5")
    print(f"{'='*70}")
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc > best_acc:
            ...
            print(f"  ✓ New best model for member {member_id+1}! (Acc: {test_acc:.2f}%)")
    ...
    print(f"\n✓ Member {member_id+1} training complete! Best acc: {best_acc:.2f}%")
    
    return best_acc


def main():
    parser = argparse.ArgumentParser(description='Deep Ensemble Training with Adam')
    ...
    parser.add_argument('--member_id', type=int, default=None,
                        help='Train specific member (0-4), or None for all')
    ...
    # Output arguments
    parser.add_argument('--save_dir', type=str, default='results/ensemble_adam')
    ...
    if args.member_id is not None:
        ...
        best_acc = train_single_member(
            args.member_id, args, device, train_loader, test_loader
        )
        print(f"\nMember {args.member_id+1} final accuracy: {best_acc:.2f}%")
    else:
        # Train all members
        best_accs = []
        ...
        for i, acc in enumerate(best_accs):
            print(f"  Member {i+1}: {acc:.2f}%")
        print(f"\n  Mean accuracy: {np.mean(best_accs):.2f}%")
        print(f"  Std deviation: {np.std(best_accs):.2f}%")
        print("="*70)
        ...
        summary = {
            ...
            'std_accuracy': float(np.std(best_accs))
        }
        with open(save_dir / 'ensemble_summary.json', 'w') as f:
